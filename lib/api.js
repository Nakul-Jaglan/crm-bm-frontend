...
export const assignmentAPI = {
  ...
  updateAssignmentStatus: async (assignmentId, status, notes) => {
    // console.log('API call - Assignment ID:', assignmentId, 'Status:', status, 'Notes:', notes);
...
// Pre-lead API
export const preLeadAPI = {
  createPreLead: async (preLeadData) => {
    const response = await api.post('/pre-leads', preLeadData);
    return response.data;
  },
  
  getPreLeads: async (skip = 0, limit = 100) => {
    const response = await api.get(`/pre-leads?skip=${skip}&limit=${limit}`);
    return response.data;
  },

  convertToLead: async (preLeadId, convertData) => {
    const response = await api.post(`/pre-leads/${preLeadId}/convert`, convertData);
    return response.data;
  },
};
